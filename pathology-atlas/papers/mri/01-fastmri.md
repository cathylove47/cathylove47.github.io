...
> 通过减少测量次数来加速磁共振成像（MRI），有潜力降低医疗成本、减轻患者的压力，并使 MRI 在目前因过于缓慢或昂贵而难以实施的应用中成为可能。我们提出 fastMRI 数据集，一个大规模集合，同时包含原始 MR 测量数据与临床 MR 图像，可用于训练与评估面向 MR 图像重建的机器学习方法。通过引入标准化的评估标准与一个可自由访问的数据集，我们的目标是帮助社区在 MR 图像重建的最先进水平上取得快速进展。我们还为没有医学影像背景的机器学习研究者提供了一份自成一体的 MRI 入门介绍。
