...
> 磁共振成像（MRI）的采集本质上很慢。受深度学习近期进展的启发，我们提出一个框架，使用深度级联的卷积神经网络从欠采样数据中重建 MR 图像，以加速数据采集过程。我们表明，对于 2D 心脏 MR 图像的笛卡尔欠采样，在 3 倍与 6 倍欠采样下，所提方法在重建误差、感知质量和重建速度方面均优于最先进的压缩感知方法，例如基于字典学习的 MRI（DLMRI）重建。与 DLMRI 相比，所提方法产生的误差大约小一半，从而能够更忠实地保留解剖结构。使用我们的方法，每幅图像可在 23 ms 内重建完成，这一速度足以支持实时应用。
