...
> 包含多种协议的 MRI 常用于诊断，但它存在采集时间长的缺点，这使得图像质量容易受到诸如运动伪影的影响。为了加速，人们提出了各种方法，从欠采样的 k-space 数据中重建出完整图像。然而，这些算法在两个主要方面存在不足。首先，在图像域中产生的混叠伪影是结构性的且非局部的，因此仅靠图像域复原是不够的。其次，尽管 MRI 在一次检查中会包含多种协议，但几乎所有先前研究都只采用单个协议的重建，把高度失真的欠采样图像作为输入，而将全采样的短协议（例如 T1）用作互补信息这一点则极少被探索。在这项工作中，我们通过提出一种嵌入了深度 T1 先验的双域循环网络（Dual Domain Recurrent Network，DuDoRNet），来同时恢复 k-space 与图像，以加速长成像协议 MRI 的采集，从而解决上述两个局限。具体而言，我们定制了一个空洞残差稠密网络（Dilated Residual Dense Network，DRDNet），用于从欠采样 MRI 数据进行双域复原。在不同采样模式和加速率上的大量实验表明，我们的方法持续优于最先进的方法，并且能够重建出高质量的 MRI。
