...
> 基于分数的扩散模型提供了一种利用数据分布梯度来对图像建模的强大方式。在这里，我们利用学习到的分数函数作为先验，引入一种从给定测量条件下的条件分布中采样数据的方式，使得该模型能够直接用于求解成像中的逆问题，尤其是加速 MRI。简而言之，我们使用去噪分数匹配训练一个连续的时间相关分数函数。随后，在推理阶段，我们在数值 SDE 求解器与数据一致性投影步骤之间迭代，以实现重建。我们的模型训练时仅需要幅度图像，却能够重建复数数据，甚至可以扩展到并行成像。所提方法对子采样模式不敏感，可与任意采样方案一起使用。此外，由于其生成式的本质，我们的方法能够量化不确定性，这是标准回归设定所无法做到的。除上述所有优点之外，我们的方法性能也非常强，甚至击败了以全监督方式训练的模型。通过大量实验，我们验证了该方法在质量与实用性方面的优越性。
