...
> 我们研究全切片图像（WSI）分类这一具有挑战性的问题。WSI 具有非常高的分辨率，且通常缺乏局部标注。当只有切片级标签可用时，WSI 分类可以被转化为一个多实例学习（MIL）问题。我们提出一种基于 MIL 的 WSI 分类与肿瘤检测方法，它不需要局部标注。我们的方法有三个主要组成部分。第一，我们引入一种新颖的 MIL 聚合器，它在一种带有可训练距离度量的双流架构中对实例之间的关系进行建模。第二，由于 WSI 可能产生庞大或不均衡的 bag，从而阻碍 MIL 模型的训练，我们提出使用自监督对比学习来为 MIL 提取良好的表征，并缓解大 bag 所面临的过高内存开销问题。第三，我们对多尺度 WSI 特征采用金字塔式融合机制，进一步提升分类与定位的准确率。我们的模型在两个具有代表性的 WSI 数据集上进行了评估。我们模型的分类准确率与全监督方法相比具有竞争力，在各数据集上的准确率差距均小于 2%。我们的结果也优于此前所有基于 MIL 的方法。在标准 MIL 数据集上的额外基准结果进一步表明，我们的 MIL 聚合器在一般 MIL 问题上具有更优表现。GitHub 仓库：https://github.com/binli123/dsmil-wsi
