...
> Vision Transformer（ViT）及其多尺度与层级变体在捕捉图像表征方面取得了成功，但它们的使用通常是在低分辨率图像上被研究的（例如 256×256、384384）。对于计算病理学中的千兆像素全切片成像（WSI），WSI 在 20X 放大倍率下可以大到 150000×150000 像素，并在不同分辨率上呈现出视觉 token 的层级结构：从捕捉细胞间空间模式的 16×16 图像，到刻画组织微环境内相互作用的 4096×4096 图像。我们引入一种新的 ViT 架构，称为层级图像金字塔 Transformer（HIPT），它利用 WSI 中固有的自然层级结构，使用两级自监督学习来学习高分辨率图像表征。HIPT 在 33 种癌症类型上进行了预训练，使用了 10,678 张千兆像素 WSI、408,218 张 4096×4096 图像和 104M 张 256×256 图像。我们在 9 个切片级任务上对 HIPT 表征进行了基准测试，并证明：1）带有层级预训练的 HIPT 在癌症亚型分类和生存预测上优于当前最先进的方法，2）自监督 ViT 能够对肿瘤微环境中表型层级结构的重要归纳偏置进行建模。
