...
> 多实例学习（MIL）已被越来越多地用于组织病理学全切片图像（WSI）的分类。然而，针对这一特定分类问题的 MIL 方法仍然面临独特的挑战，尤其是那些与样本队列规模小相关的挑战。在这类问题中，WSI 切片（bag）的数量有限，而单张 WSI 的分辨率极为庞大，这导致从这张切片中裁剪出大量 patch（instance）。为解决这一问题，我们提出通过引入伪包（pseudo-bag）的概念来虚拟地扩大 bag 的数量，并在其之上构建了一个双层 MIL 框架，以有效利用内在特征。此外，我们还在基于注意力的 MIL 框架下推导实例概率方面做出了贡献，并利用该推导来帮助构建和分析所提出的框架。所提方法在 CAMELYON-16 上以大幅度的优势优于其他最新方法，在 TCGA 肺癌数据集上也具有更好的性能。所提出的框架易于扩展以用于更广泛的 MIL 应用。代码可在以下地址获取：https://github.com/hrzhang1123/DTFD-MIL
