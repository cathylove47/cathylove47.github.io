...
> 对比式视觉语言预训练已成为一种强有力的方法，既可用于训练新的语言感知图像编码器，也可用于为现有预训练模型增强零样本视觉识别能力。然而，现有工作通常在大规模图文对数据集上训练，并且被设计为执行仅涉及中小尺寸图像的下游任务；这两点都不适用于计算病理学这一新兴领域，因为在该领域中公开可用的配对图文数据集有限，而且每张图像可跨越多达 100,000 × 100,000 像素。在本文中，我们提出 MI-Zero，一个简单而直观的框架，用于在千兆像素组织病理学全切片图像上释放对比对齐的图像模型与文本模型的零样本迁移能力，使多个下游诊断任务能够由预训练编码器完成，而无需任何额外标签。MI-Zero 在多实例学习的框架下重新表述零样本迁移，以克服在极大图像上进行推理的计算挑战。我们使用超过 55 万份病理报告及其他可获得的域内文本语料来预训练我们的文本编码器。通过有效利用强大的预训练编码器，我们基于超过 3.3 万对组织病理学图文对预训练的最佳模型，在三个不同的真实世界癌症亚型分类任务上取得了 70.2% 的平均中位零样本准确率。我们的代码可在以下地址获取：https://github.com/mahmoodlab/MI-Zero
