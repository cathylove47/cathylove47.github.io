...
> 整合全切片图像（WSIs）与 bulk 转录组学来预测患者生存，能够增进我们对患者预后的理解。然而，由于这些数据的性质不同，这一多模态任务尤为具有挑战性：WSIs 表示对肿瘤的极高维空间描述，而 bulk 转录组学表示对该肿瘤内基因表达水平的全局描述。在此背景下，我们的工作旨在应对两个关键挑战：（1）我们如何才能以语义上有意义且可解释的方式对转录组学进行 token 化？以及（2）我们如何才能捕捉这两种模态之间稠密的多模态交互？具体而言，我们提出从转录组学中学习能够编码特定细胞功能的生物通路 token。我们主张，这些 token 与编码 WSI 中不同形态学模式的组织学 patch token 一起，构成了适合下游可解释性分析的推理单元。我们提出使用一种内存高效的多模态 Transformer 来融合这两种模态，该 Transformer 能够对通路 token 与组织学 patch token 之间的交互进行建模。我们提出的模型 SURVPATH，在来自 The Cancer Genome Atlas 的五个数据集上，与单模态和多模态基线进行评估时，取得了 state-of-the-art 的性能。我们的可解释性框架识别出关键的多模态预后因素，因此能够为基因型与表型之间的交互提供有价值的洞见，从而更深入地理解其中所涉及的潜在生物学机制。我们在以下地址公开我们的代码：https://github.com/ajv012/SurvPath。
