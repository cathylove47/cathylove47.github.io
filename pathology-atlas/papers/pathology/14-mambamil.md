...
> 多实例学习（MIL）已成为计算病理学中在全切片图像（WSIs）内提取判别性特征表示的主流范式。尽管推动了显著的进展，现有的 MIL 方法在促进实例之间全面且高效的交互方面仍存在局限，也面临与耗时计算和过拟合相关的挑战。在本文中，我们将选择性扫描空间状态序列模型（Mamba）引入多实例学习（MIL）中，以线性复杂度进行长序列建模，称之为 MambaMIL。通过继承原版 Mamba 的能力，MambaMIL 展现出全面理解并感知长实例序列的能力。此外，我们提出了感知实例顺序与分布的序列重排 Mamba（SR-Mamba），它能够利用长序列中内蕴的有价值信息。以 SR-Mamba 作为核心组件，MambaMIL 能够有效捕捉更具判别性的特征，并缓解与过拟合和高计算开销相关的挑战。在两个公开的挑战性任务、跨越九个多样数据集上的大量实验表明，我们提出的框架相较 state-of-the-art MIL 方法表现更优。代码发布于 https://github.com/isyangshu/MambaMIL。
