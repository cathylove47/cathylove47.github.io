...
> 准确建模多类细胞拓扑在数字病理学中至关重要，因为它为组织结构与病理提供了关键洞见。细胞拓扑的合成生成，能够实现对复杂组织环境的真实模拟，通过增强训练数据提升下游任务，更贴近病理医生的领域知识，并为控制与泛化肿瘤微环境提供了新的机会。在本文中，我们提出一种新颖方法，将拓扑约束整合进扩散模型，以改进真实且上下文准确的细胞拓扑的生成。我们的方法精炼了对细胞分布与相互作用的模拟，提升了下游任务（如细胞检测与分类）结果的精度与可解释性。为评估生成布局的拓扑保真度，我们提出一个新指标——拓扑 Fréchet 距离（TopoFD），它克服了 FID 等传统指标在评估拓扑结构上的局限。实验结果表明，我们的方法在生成能够捕捉复杂拓扑关系的多类细胞布局方面是有效的。代码见 https://github.com/Melon-Xu/TopoCellGen。
