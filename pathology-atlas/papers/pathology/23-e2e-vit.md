...
> 传统的全切片图像（WSI）分析流程遵循两阶段过程。首先，使用一个图像编码器，例如视觉 transformer（ViT），对从 WSI 裁切出的一系列 tile 执行批量离线特征提取。其次，用切片级标签训练一个多实例学习（MIL）模型，以获得任务特定的切片嵌入。然而，这存在若干局限：对 tile 编码器预训练权重的强依赖、缺少来自原始图像的感受野，以及缺乏任务无关的 WSI 表征。一个理想的改进是开发一个端到端预训练 WSI 模型，但从零训练它将面临高训练成本与计算复杂度等挑战。在这项工作中，我们解构基于 ViT 的病理图像表征的关键步骤，并提出一种称为 E2E-ViT 的转换策略，它把一个朴素 ViT 转换为端到端预训练 WSI 模型，且不引入额外参数。E2E-ViT 直接输入 WSI 中的整个组织区域，以高效地将图像序列送入 transformer 主干，从原始感受野实现信息交互并生成切片特征。通过多个生存预测任务，我们证明转换后的预训练 ViT 优于两阶段 MIL 模型与切片基础模型（SFM）。我们的工作提出了一种新的端到端学习范式，为下一代计算病理学模型提供了一个有前景的方向。
