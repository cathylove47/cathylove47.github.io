...
> 准确检测癌组织区域（CTR）使得对肿瘤微环境进行更深入的分析成为可能，并为治疗反应提供关键洞见。传统的 CTR 检测方法通常依赖组织学图像中丰富的细胞形态，由于不同组织区域之间存在形态相似性，它们容易产生较高的假阳性率。空间转录组学（ST）的突破性进展提供了详细的细胞表型与空间定位信息，为更准确的癌区检测提供了新机会。然而，现有方法无法有效地将组织学图像与 ST 数据整合起来，尤其是在面向 CTR 检测的跨样本与跨平台/批次设定下。为应对这一挑战，我们提出 SpaCRD，一种基于迁移学习的方法，它深度融合组织学图像与 ST 数据，从而在多样化的样本、平台与批次之间实现可靠的 CTR 检测。一旦在源数据上完成训练，SpaCRD 便能轻松泛化，以在不同平台与批次的样本间准确检测癌区。SpaCRD 的核心是一个类别正则化、变分重建引导的双向交叉注意力融合网络，它使模型能够从多个视角自适应地捕捉组织学特征与基因表达之间的潜在共表达模式。在横跨多种疾病类型、平台与批次的 23 个配对的组织学—ST 数据集上进行的大量基准分析表明，SpaCRD 在 CTR 检测上持续优于现有的八种最先进方法。
