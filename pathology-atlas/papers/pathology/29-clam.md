...
> 计算病理学这一快速兴起的领域，有潜力实现客观诊断、治疗反应预测，以及识别具有临床相关性的新形态学特征。然而，基于深度学习的计算病理学方法，要么需要在全监督设定下对千兆像素级全切片图像（WSIs）进行人工标注，要么在弱监督设定下需要数千张带切片级标签的 WSIs。此外，切片级计算病理学方法还受困于域自适应与可解释性问题。这些挑战阻碍了计算病理学在临床与研究用途上的广泛采用。在此，我们提出 CLAM——聚类约束注意力多实例学习（Clustering-constrained attention multiple instance learning），一种易用、高通量且可解释的 WSI 级处理与学习方法，它仅需切片级标签，同时具备数据高效性、可适应性，并能处理多类亚型分类问题。CLAM 是一种基于深度学习的弱监督方法，它使用基于注意力的学习来自动识别具有高诊断价值的子区域，从而准确地对整张切片进行分类；同时，它还利用在所识别出的代表性区域上进行的实例级聚类，来约束并精炼特征空间。在三项独立的分析中，我们展示了 CLAM 的数据高效性与可适应性，以及其相较标准弱监督分类的更优表现。我们证明 CLAM 模型是可解释的，并可用于识别已知的与新的形态学特征。我们进一步表明，使用 CLAM 训练的模型能够适应独立的测试队列、手机显微成像以及活检样本。CLAM 是一种通用且可适应的方法，可用于临床与研究场景中各种不同的计算病理学任务。
