...
> 多实例学习（MIL）是解决基于全切片图像（WSI）的病理诊断中弱监督分类的一种强大工具。然而，当前的 MIL 方法通常基于独立同分布假设，因而忽略了不同实例之间的相关性。为解决这个问题，我们提出了一个新框架，称为相关 MIL（correlated MIL），并给出了收敛性证明。基于该框架，我们设计了一种基于 Transformer 的 MIL（TransMIL），它同时探索了形态学信息与空间信息。所提出的 TransMIL 能够以出色的可视化与可解释性，有效地处理不平衡/平衡以及二分类/多分类。我们针对三个不同的计算病理学问题开展了多种实验，与最先进的方法相比取得了更好的性能和更快的收敛。在 CAMELYON16 数据集上，肿瘤二分类的测试 AUC 最高可达 93.09%。而在癌症亚型分类上，AUC 在 TCGA-NSCLC 数据集和 TCGA-RCC 数据集上分别最高可达 96.03% 和 98.82%。实现代码可在以下地址获取：https://github.com/szc19990412/TransMIL。
