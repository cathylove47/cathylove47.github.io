...
> 数字病理学带来了独特的计算挑战，因为一张标准的千兆像素切片可能包含数万个图像图块 1 – 3 。此前的模型往往对每张切片仅下采样一小部分图块，因而遗漏了重要的切片级上下文 4 。在此，我们提出 Prov-GigaPath，一个整张切片病理学基础模型，它在来自 Providence 的 171,189 张整张切片中的 13 亿个 256 × 256 病理图像图块上预训练；Providence 是一个由 28 个癌症中心组成的大型美国医疗网络。这些切片来自超过 30,000 名患者，涵盖 31 种主要组织类型。为预训练 Prov-GigaPath，我们提出 GigaPath，一种用于预训练千兆像素病理切片的全新视觉 Transformer 架构。为将 GigaPath 扩展到可对含数万个图像图块的切片进行切片级学习，GigaPath 将新近开发的 LongNet 5 方法适配到数字病理学。为评估 Prov-GigaPath，我们构建了一个数字病理学基准，包含 9 个癌症亚型任务与 17 个病理组学任务，同时使用 Providence 与 TCGA 数据 6 。凭借大规模预训练与超大上下文建模，Prov-GigaPath 在 26 个任务中的 25 个上达到最先进的性能，并在 18 个任务上相较第二好的方法有显著提升。我们进一步通过纳入病理报告，展示了 Prov-GigaPath 在面向病理学的视觉—语言预训练上的潜力 7 , 8 。总之，Prov-GigaPath 是一个开放权重的基础模型，在各种数字病理学任务上取得最先进的性能，证明了真实世界数据与整张切片建模的重要性。
