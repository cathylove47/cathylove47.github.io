...
> 全切片图像（WSI）分类通常依赖深度弱监督多实例学习（MIL）方法来处理千兆像素分辨率的图像与切片级标签。然而，深度学习的良好表现源自对海量数据集与多样样本的利用，这就迫切需要能够扩展到大规模数据集的高效训练流程，以及用于使样本多样化的数据增强技术。但是，当前基于 MIL 的 WSI 分类流程内存开销大且计算效率低，因为它们通常将数万个 patch 组装成 bag 进行计算。另一方面，尽管数据增强在其他任务中很流行，但在 WSI MIL 框架中尚未被探索。为解决这些问题，我们提出 ReMix，一个面向基于 MIL 的 WSI 分类的通用且高效的框架。它包含两个步骤：reduce 与 mix。首先，它通过用实例原型（instance prototype，即 patch 聚类质心）替代实例，来减少 WSI bag 中的实例数量。然后，我们提出一种“Mix-the-bag”增强，它包含四种在线、随机且灵活的隐空间增强。它在隐空间中带来多样且可靠、保持类别身份的语义变化，同时强制实现语义扰动不变性。我们在两个公开数据集上、用两种最先进的 MIL 方法评估 ReMix。在我们的实验中，精确率、准确率与召回率均取得一致提升，同时训练时间与内存消耗降低数个数量级，证明了 ReMix 的有效性与高效性。代码已公开。
