...
> 全切片图像是数字病理学用于癌诊断与治疗的基础。撰写病理报告对经验不足的病理医师而言既费力又容易出错。为减轻工作量并提升临床自动化，我们研究如何在给定全切片图像的情况下生成病理报告。在数据方面，我们整理出了最大的 WSI—文本数据集（PathText）。具体而言，我们通过识别并清洗 TCGA 中描述诊断切片的病理报告，为视觉—语言模型收集了近 10000 对高质量 WSI—文本对。在模型方面，我们提出了多实例生成模型（MI-Gen），它能为千兆像素级 WSI 生成病理报告。我们在 TCGA-PathoText 的最大子集上对我们的模型进行了基准测试。实验结果表明，我们的模型能够生成包含多个临床线索的病理报告，并在某些切片级任务上取得有竞争力的性能。我们观察到，从病理报告中进行简单的语义抽取，就能在 BRCA 亚型分类上取得最佳性能（F1 分数 0.838），超越此前 state-of-the-art 的方法。我们收集的数据集及相关代码已公开可用。
