...
> 全切片图像（WSIs）在数字病理学中至关重要，它使千兆像素级的组织分析能够覆盖各种病理任务。尽管多模态大语言模型（MLLMs）的最新进展允许通过自然语言进行多任务 WSI 分析，但它们常常不及任务专用模型。协作式多智能体系统已成为在医疗保健中平衡通用性与准确性的一种有前景的方案，但其潜力在病理学特定领域仍未得到充分探索。为解决这些问题，我们提出 WSI-Agents，一种用于多模态 WSI 分析的新型协作式多智能体系统。WSI-Agents 将专门的功能智能体与稳健的任务分配和核验机制相结合，通过三个组件同时提升任务专用准确性与多任务通用性：(1) 一个任务分配模块，利用由 patch 级与 WSI 级 MLLMs 组成的模型库将任务分配给专家智能体；(2) 一个核验机制，通过内部一致性检查，以及利用病理知识库和领域专用模型进行的外部验证来确保准确性；(3) 一个总结模块，结合可视化解释图综合出最终总结。在多模态 WSI 基准上的大量实验表明，WSI-Agents 在多种任务上都优于当前的 WSI MLLMs 和医疗智能体框架。
