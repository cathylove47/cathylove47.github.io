...
> 空间转录组学（ST）在基因表达与组织形态之间架起桥梁，但因技术复杂性和高昂成本而面临临床采用的障碍。尽管计算方法可从 H&E 染色的全切片图像（WSIs）预测基因表达，但现有方法往往无法捕捉 spot 内部复杂的生物学异质性，并且在整合来自周围组织的上下文信息时易受形态学噪声干扰。为克服这些局限，我们提出 HiFusion，一种整合了两个互补组件的新型深度学习框架。首先，我们引入分层 spot 内建模（Hierarchical Intra-Spot Modeling）模块，它通过多分辨率的子块分解提取细粒度形态表征，并由一个特征对齐损失引导，以确保跨尺度的语义一致性。与此同时，我们提出上下文感知的跨尺度融合（Context-aware Cross-scale Fusion）模块，它采用 cross-attention 选择性地纳入具有生物学相关性的区域上下文，从而增强表征能力。该架构能够对细胞级特征和组织微环境线索进行全面的建模，而这两者对于准确的基因表达预测都必不可少。在两个基准 ST 数据集上的大量实验表明，HiFusion 在 2D 按切片交叉验证以及更具挑战性的 3D 按样本场景中都取得了 state-of-the-art 性能。这些结果凸显了 HiFusion 作为一种稳健、准确且可扩展方案，从常规组织病理学进行 ST 推断的潜力。
