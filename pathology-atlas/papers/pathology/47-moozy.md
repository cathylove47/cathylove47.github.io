...
> 计算病理学需要能够迁移到多样临床任务的全切片图像（WSI）基础模型，然而当前方法在很大程度上仍以切片为中心，往往依赖私有数据与昂贵的配对报告监督，并且没有显式建模来自同一患者的多个切片之间的关系。我们提出 MOOZY，一个患者优先的病理基础模型，其中患者病例而非单张切片，是表征的核心单位。MOOZY 在预训练期间通过一个 case transformer 显式建模来自同一患者所有切片之间的依赖关系，将多阶段自监督与规模化低成本任务监督相结合。在第一阶段，我们使用掩码自蒸馏在 77,134 个公共切片特征网格上预训练一个仅视觉的切片编码器。在第二阶段，我们使用一个 case transformer 以及对来自 56 个公共数据集的 333 个任务的多任务监督，将这些表征与临床语义对齐，其中包含跨四个终点的 205 个分类任务与 128 个生存任务。在十六个留出任务上，MOOZY 相对 PRISM 分别将 macro weighted F1、balanced accuracy 与 macro weighted ROC-AUC 提升 +4.19%、+7.93% 与 +6.95%。MOOZY 在参数上也很高效，拥有 85.77M 参数，比 GigaPath 小 14×。这些结果表明，患者级预训练能产生可迁移的嵌入，为可扩展的患者优先组织病理学基础模型提供了一条路径。
