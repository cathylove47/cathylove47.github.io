...
> 视觉 Transformer（ViTs）及其层级变体在计算病理学（CPath）中取得了强劲的性能。然而，它们大多在单一倍率的全切片图像（WSIs）上预训练，限制了其在任意倍率上的泛化能力。千兆像素级 WSIs 天生包含多个尺度上的诊断模式，包括细胞形态、组织结构与全局上下文，这与专家病理学家检视 WSIs 的方式相呼应。我们提出多倍率金字塔 Transformer（Multi-Resolution Pyramid Transformer，MRPT），一个从细胞到组织与 WSI 层级逐级聚合多倍率信息的模型。MRPT 采用一种具有生物学意义的连续跨倍率注意力（Consecutive Cross-Resolution Attention，CCRA）机制来捕捉与尺度无关的交互，并通过跨倍率对齐嵌入来强制多倍率语义一致性，从而产生稳健且可泛化的 WSI 表征。MRPT 以多倍率自监督方式在 6.24 亿个 patch、240 万个区域与 3.6 万张 WSIs 上预训练，学习到丰富的由粗到细的组织病理学特征。在 34 个多样数据集上的大量实验表明，MRPT 在癌种亚型分类、组织表型分析以及用于 WSI 理解的视觉问答（VQA）中，超越了近期的基础模型与多模态大语言模型（MLLMs）。
